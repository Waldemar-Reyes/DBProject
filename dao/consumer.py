...
class ConsumerDAO:
    ...
    def getOrdersByConsumerId(self, consid):
        cursor = self.conn.cursor()
        query = "select consid, consusername, odid, odnumber, odtime from orders natural inner join makes natural inner join consumer where consid = %s;"
        cursor.execute(query, (consid,))
        result = []
        for row in cursor:
            result.append(row)
    ...
    def getPayMethodByConsumerId(self, consid):
        cursor = self.conn.cursor()
        query = "select consid, consusername, pmid, pmname from pay_method natural inner join owns natural inner join consumer where consid = %s;"
        cursor.execute(query, (consid,))
        result = []
        for row in cursor:
            result.append(row)
    ...
    def getReservationByConsumerId(self, consid):
        cursor = self.conn.cursor()
        query = "select consid, consusername, resid, resname, restype, resprice, resstock, reslocation, restime from reservation natural inner join requests natural inner join consumer where consid = %s;"
        cursor.execute(query, (consid,))
        result = []
        for row in cursor:
            result.append(row)
    ...
    def getResourcesByConsumerId(self, consid):
        cursor = self.conn.cursor()
        query = "select consid, consusername, rid, rname, rtype, rprice, rstock, rlocation from resources natural inner join asks natural inner join reservation natural inner join requests natural inner join consumer where consid = %s;"
        cursor.execute(query, (consid,))
        result = []
        for row in cursor:
            result.append(row)
